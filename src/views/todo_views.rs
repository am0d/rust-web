...
pub struct TodoDetailView<'self> {
    model: &'self Todo
}

impl<'self> TodoDetailView<'self> {
    pub fn new(m: &'self Todo) -> TodoDetailView<'self> {
        TodoDetailView {
            model: m//.clone()
        }
    }
...
impl<'self> View for TodoDetailView<'self> {
    fn render(&self, print: |&SafeHtmlString| -> ()) {
        print(&SafeHtmlString::new("<!DOCTYPE html>
<html lang=\"en\">
  <head>
